  ...
  <div class="contbody">
    ...
    <div class="FromDataBox" style="overflow:hidden;padding-top:24px;">
      ...
      >
        ...
        <FormItem label="选择模型：" prop="modelChoice">
          <Select v-model="formVal.modelChoice">
            <Option
              ...
              :id="item.id"
              :key="index"
              @click.native="descriptionBtn(item.description)"
        ...
        <FormItem class="fromClass_1" label="数据文件：" prop="formData">
          ...
          <div style="width:138px;margin-left:20px;display: flex;">
            <Upload
              ...
              action="api/baike/tyzx/create_task"
            >
        ...
        <div
          style="line-height:34px;font-size:color:#60636f;font-size:12px;margin-bottom:10px;margin-left:98px;"
        >
        ...
        <FormItem
          style="width:100%;margin-top:30px;margin-bottom:50px;display: flex;justify-content: center;"
        >
          <Button type="primary" @click="handleSubmit('formVal')"
            >创建任务</Button
          >
          <Button style="margin-left: 8px" @click="gotoHome()">返回</Button>
        </FormItem>
      </Form>
    </div>
  </div>
    ...
    handleSubmit(name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          // 添加请求头
          axios
            .post("api/baike/tyzx/create_task", param, config)
            .then((res) => {

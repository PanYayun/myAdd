<template>
  <div>
    <Modal v-model="isShowTree" width="70%" title="内场质量问题详情">
          ...
          <div v-for="(item, index) in tableList" :key="index">
            ...
            <div v-if="item.type == 1">
              <div class="flex" v-if="isEmpty(item.value)">
                ...
                <div
                  v-else
                  class="table-item  table-item75"
                  v-html="processValue(item.key, item.value)"
                ></div>
              </div>
            </div>
            <div class="flex" v-if="item.type == 2">
              ...
              <div class="flex table-item50" v-if="isEmpty(item.value2)">
                ...
                <div
                  class="table-item  table-item50"
                  v-html="processValue(item.key2, item.value2)"
                ></div>
              </div>
            </div>
          </div>
...
export default {
  ...
  props: ["failureRecord", "rightImgText3"],
  components: {
    collapse,
  },
  data() {
    ...
    gettableListData(newVal) {
      ...
      if (true) {
        this.tableList = [
          ...
          {
            key: "现象主体",
            value: newVal.xxzscq_xxzt,
            type: 2,
            key2: "试验项目",
            value2: newVal.xxzscq_syxm,
          },
          {
            key: "试验类型",
            value: newVal.xxzscq_sylx,
            type: 2,
            key2: "环境指标",
            value2: newVal.xxzscq_hjzb,
          },
          {
            key: "故障模式",
            value: newVal.fualtMode,
            type: 1,
          },
          {
            key: "技术要求指标",
            value: newVal.xxzscq_jsyqzb,
            type: 2,
            key2: "实测不合格指标",
            value2: newVal.xxzscq_scbhgzb,
          },
          {
            key: "填报人",
            value: newVal.tbr,
          ...
          {
            key: "产品处理意见",
            value: newVal.czyjjjzcs_cpclyj,
            type: 1,
          },
          {
            key: "产品处理意见详细说明",
            value: newVal.czyjjjzcs_cpclxxsm,
            type: 1,
          },
          {
            key: "举一反三",
          ...
          {
            key: "批次",
            value: newVal.czyjjjzcs_jyfs_pc,
            type: 2,
            key2: "数量",
            value2: newVal.czyjjjzcs_jyfs_sl,
          },
          {
            key: "举一反三详细说明",
            value: newVal.czyjjjzcs_jyfs_xxsm,
            type: 1,
          },
          {
            key: "故障件处理意见",
            value: newVal.czyjjjzcs_gzjclyj,
            type: 1,
          },
          {
            key: "故障件处理意见详细说明",
            value: newVal.czyjjjzcs_gzjcl_xxsm,
            type: 1,
          },
          {
            key: "重检记录",
          ...
          {
            key: "记录情况",
            value: newVal.zjxx_jlqk,
            type: 2,
            key2: "检验",
            value2: newVal.zjxx_jy,
          },
          {
            key: "日期",
            value: newVal.zjxx_rq,
            type: 1,
          },
          {
            key: "故障机理分类情况",

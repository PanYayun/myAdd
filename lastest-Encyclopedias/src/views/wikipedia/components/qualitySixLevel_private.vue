<template>
  <div class="qualitySixLevel">
    <div
      class="four_level"
      v-for="(item5, index5) in forLevelItem2"
      :key="'five' + index5"
    >
      <div class="flex vertical_center" v-if="checkfloorIsShow(item5)">
        <label
          style="flex: 1"
          class="four_level_label"
          @click="clickPushDetail5(item5)"
        >
          {{ item5.Fault_Name }}
        </label>
        <div
          class="flex vertical_center show_type"
          @click="clickMore(item5)"
          v-if="item5.isShowMore"
        >
          <span>{{ item5.isExpand ? "收起" : "展开" }}</span>
          <i
            class="iconfont icon-shouqi "
            :class="{ 'down-rotate': !item5.isExpand }"
          ></i>
        </div>
      </div>
      <div
        class="five_level flex "
        :class="'standard_label' + group_index"
        :id="index5"
        :style="{ height: item5.standard_label_height }"
      >
        <label
          class="title"
          v-for="(item6, index6) in item5.Lower_Fault_List"
          :key="'six' + index6"
          @click="clickPushDetail6(item6)"
          >{{ item6.Fault_Name }}</label
        >
      </div>
    </div>
  </div>

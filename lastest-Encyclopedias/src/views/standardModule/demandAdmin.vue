      ...
      <div class="content_r">
        ...
        <div class="sort_module sort_btn">
          <div class="sort_btn_l">
            <div>
              （已选
              <span style="color:#3388ff">{{ this.selection.length }}</span>
              条）
            </div>
            <RadioGroup v-model="valid" @on-change="changeStatus()">
...
export default {
  ...
  data() {
    return {
      ...
      // 顶部列表信息
      tabList: [
        { text: "集团标准体系", treeApi: "standard", stdType: 1 },
        { text: "国军标体系", treeApi: "standard", stdType: 0 },
        { text: "832A", treeApi: "standard", stdType: 3 },
        { text: "832", treeApi: "standard", stdType: 2 },
      ],
      sortList: [
        //排序列表
        { text: "发布日期", key: "pub_date" },
        // { text: "智能排序", key: "implement_date" },
      ],
      treeVal: "", //左侧树的关键字搜索
      treeVal2: "", //右侧树的关键字搜索
  ...
  methods: {
    ...
    generateReport() {
      ...
      if (this.selection.length > 0) {
        ...
        const { stdType } = this.tabList[this.tabIndex];
        const stdNos = this.selection.map((item) => item.id).join(",");
        console.log(stdNos, "stdNos");
    ...
    listData() {
      ...
      this.$axios(stdDemandQuery, "get", parameter).then((res) => {
        -setTimeout(() => {
          this.spinShow = false;
        }, 500);
        const { data } = res;

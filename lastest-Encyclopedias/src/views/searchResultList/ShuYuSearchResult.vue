    ...
    <ul class="searchUl">
      <li v-for="(item, index) of list" :key="index" @click="touchRow(item)">
        <div class="searchUl_t">
          ...
          <span class="firstClass">{{ item.YJFL }}</span>
          <span class="secondClass">{{ item.EJFL }}</span>
        </div>
        <div class="searchUl_b">{{ item.YSCTSW }}</div>
      </li>
    </ul>
  ...
  methods: {
    loadData() {
      ...
      if (this.SearchImg.status === "0") {
        this.$axios(
          "/baike/hangkongshuyu/search_by_class",
          "post",
          this.$qs.stringify({
            thisClass: this.$route.query.id, //当前分类ID
        ...
        this.$axios(
          "/baike/hangkongshuyu/search_by_keyword",
          "post",
          this.$qs.stringify({

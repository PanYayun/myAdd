  ...
  <div class="flex" ref="htmlTxt">
    <div class="left_module">
        ...
        <div class="lemmaWgt_lemmaTitle flex vertical_center">
          ...
          <ul class="handle_lemma flex">
            <li class="disable-color">
              ...
              <span class="disable-color-text">语音</span>
            </li>
            <li class="disable-color">
              <i class="iconfont icon-bianji"></i>
              <span class="disable-color-text">编辑</span>
            </li>
            <li class="disable-color">
              <i class="iconfont icon-xinxi"></i>
              <span class="disable-color-text">讨论</span>
            </li>
            <li class="disable-color">
              <i class="iconfont icon-zengjia-cuxiantiao"></i>
              <span class="disable-color-text">上传视频</span>
            </li>
          </ul>
          <div @click="isTrue">
      ...
      <!-- <div class="edit_container">
        ...
        <Button type="primary" class="confirm_sub" @click="submitData" style="margin-top: 10px;">确认提交</Button>
      </div> -->
      <div style="margin-top: 16px;" v-for="(item, index) in subDetailData.middleContent" :key="index">


        <div v-for="(item2, index2) in item.middleBox" :key="index + index2"
          ...
          <div class="ifififi" v-if="item2.Text_Type == 'Text&Picture'">
            <div class="unique_wrap fun_diagram padding_0">
              <div class="unique_title_text flex">
                ...
                <div class="show_type vertical_center flex" @click="clickzhankai(item2)">
                  <span>{{
                    item2.isOpen ? "收起" : "展开"
                  }}</span>
                  ...
                  <table>
                    <tr v-for="(table_item, index3) in item2.Text_Content" :key="index + index2 + index3">
                      <td style="width:150px;">
                        <div
                          style="width:150px;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;word-break: break-all;text-align: left;padding-left: 25px;">
                          {{ getMapKeyAndValueWithString(table_item, "k") }}
                        </div>
                      </td>
                      <td>
                        <div
                          style="white-space: nowrap;text-overflow: ellipsis;overflow: hidden;word-break: break-all;text-align: left;padding-left: 25px;">
          ...
          <div class="ifififi" v-if="item2.Text_Type == 'text' || item2.Text_Type == 'Text&text'">
            <div class="unique_wrap padding_0">
              <div class="unique_title_text flex" style="margin-bottom: 17px;">
                <span>{{ item2.Text_title }}</span>
                <div class="show_type vertical_center flex" @click="clickzhankai(item2)">
          ...
          <div class="ifififi" v-if="
              item2.Text_Type == 'MapInList' || item2.Text_Type == 'MapInList2'
            ">
            <div class="unique_wrap padding_0">
              ...
              <div>
                <Form inline ref="table_form_1" :rules="faultList.form_rules_1" :model="faultList.table_form_1"
                  label-position="left">
                  <FormItem label="起至时间" prop="selectTime" :label-width="80">
                    <DatePicker v-model="faultList.table_form_1.selectTime" type="daterange" placement="bottom-end"
                      placeholder="请选择时间" style="width: 100%"></DatePicker>
                  </FormItem>
                  <FormItem label="是否有归零" prop="searchName" :label-width="90">
                    <Select v-model="faultList.table_form_1.selectType" placeholder="请选择类型" style="width: 100px">
                      <Option v-for="item in faultList.typeList" :value="item.value" :key="item.value">{{ item.label }}
                      </Option>
                    </Select>
                  </FormItem>
              ...
              <div class="table" v-if="privateShow">
                <Table width="840" border stripe :columns="xhgzcolumns" :data="faultList.data" no-data-text="暂无内容">
                </Table>
              </div>
              <div class="table" v-else>
                <Table width="840" border stripe :columns="xhgzcolumns_private" :data="faultList.data"
                  no-data-text="暂无内容"></Table>
              </div>
              <Row v-if="spin_search_Page_box">
                ...
                <div style="padding-top: 15px;display: flex;justify-content: center;align-items: center;">
                  ...
                  <div>
                    <Page :total="totalCount" :current="table_current_1" :page-size="5"
                      :page-size-opts="[5, 10, 20, 30]" @on-change="table_Pagechange_1"
                      @on-page-size-change="table_PageSizechange_1" show-sizer show-elevator></Page>
                  </div>
                </div>
                </Col>
              </Row>
    ...
    <div class="right_module">
      ...
      <!-- 相似产品词条 -->
      <div class="termExplain termExplain" v-if="search_term.data.length > 0">
          ...
          <div class="item flex" v-for="(item, index) in search_term.data" :key="index">
            <Tooltip style="width: 100%;" max-width="350" theme="light" :content="
              ...
              <div class="item flex item_hover" @click="didClickTerm(item)">
                ...
                <span
                  style="width: 50%;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">释义来源：{{ item.source || "暂无数据" }}</span>
              </div>
            </Tooltip>
      ...
      <div class="standardKnowledge" v-if="search_standard.data.length > 0">
        ...
        <div class="list">
          <div class="item flex" v-for="(item, index) in search_standard.data" :key="index"
            @click="didClickstandardKnowledge(item)">
      ...
      <div class="restsRecommend" v-if="search_other.data.length > 0">
        ...
        <div class="list">
          <div class="item" v-for="(item, index) in search_other.data" :key="index"
            @click="didClickrestsRecommend(item)">
            <Tooltip max-width="350" theme="light" :content="item.title">
    ...
    <failure-record-model v-if="privateShow" :failureRecord="table_current_item" ref="failureRecord">
    </failure-record-model>
    <failure-record-model-private v-else :failureRecord="table_current_item" :rightImgText3="rightImgText3"
      ref="failureRecord"></failure-record-model-private>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    props: ["detailData", "rightImgText", "rightImgText2", "rightImgText3"],
    components: {
      collapse,
      failureRecordModel,
    ...
    methods: {
      ...
      onEditorBlur(editor) {
        // 失去焦点事件
        console.log(this.editContent, '111ppppppp')
      },
      onEditorFocus(editor) {
        this.editContent="<span style='color:red;'>hello baby</span>"
        // 当前所有页面html
        let currentHtml=this.$refs['htmlTxt']['__vue__'].$el.innerHTML;
        console.info(this.$refs['htmlTxt']['__vue__'].$el.innerHTML,'ssssssss')
        // 获得焦点事件
        console.log('222')
      },
      ...
      guzhangProcessingData() {
        ...
        this.detailData.middleContent.forEach((middleContent) => {
          middleContent.middleBox.forEach((middleBox, index) => {
            if (
              middleBox.Text_Type == "MapInList" ||
              middleBox.Text_Type == "MapInList2"
            ) {
              this.faultList.data = middleBox.Text_Content;
              this.totalCount = middleBox.Count_Intems;
      ...
      searchXXGZ() {
        ...
        } else if (this.faultList.table_form_1.selectType == "2") {
          this.faultList.table_form_1.faultZeroingAnalysis = false;
        } else if (this.faultList.table_form_1.selectType == "0") {
          this.faultList.table_form_1.faultZeroingAnalysis = "";
        }
        //处理故障类型
      ...
      didClickrestsRecommend(val) {
        this.didClickRecommendDetail(val);
      },
      didClickstandardKnowledge(val) {
        this.didClickRecommendDetail(val);
      },
      didClickRecommendDetail(val) {
      ...
      WikipediaDetails: {
        handler(newVal, oldVal) {
          if (
            ...
            this.loadAllData();
            this.isTrue();
            if (this.rightImgText2 == "通用故障") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/tygz.jpg")];
            } else {
              this.rotate_Pictures_urls = [];
            }
            if (this.rightImgText == "后减速板") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/hjsb.png")];
            } else if (this.rightImgText == "集成电路") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/jcdl.jpg")];
            } else if (this.rightImgText == "钽电解电容") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/tdjdr.jpg")];
            } else if (this.rightImgText == "XX8") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/XXX8.jpg")];
            } else if (this.rightImgText == "机体结构") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/jtjg.jpg")];
            } else if (this.rightImgText == "控制电路") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/kzdl.jpg")];
            } else if (this.rightImgText == "输出信号异常") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/scxhyc.png")];
            } else if (this.rightImgText == "舵机-XXA") {
              this.rotate_Pictures_urls = [require("@/img/jj_banner/dj.jpg")];
            }
          }
        },
        // 加上之后第一次定义时就会执行

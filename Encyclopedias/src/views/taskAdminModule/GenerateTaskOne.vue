  ...
  >
    <Card :bordered="false" dis-hover :padding="0">
      <Row>
        ...
        >
          <p style="font-size: 18px; font-weight: 800">事理图谱生成过程</p>
        </Col>
        ...
        <Col span="24" style="padding:  15px 0;">
          <Button type="primary" style="width: 100px" @click="NextSteps()"
            >下一步</Button
          >
          <Button type="primary" style="width: 100px;margin-left:20px" @click="ReturnSuperior()"
            >返回</Button
          >
        </Col>
...
export default {
  ...
  methods: {
    ...
    TableData_1: _debounce(function() {
      this.loading_1 = true;
      this.$axios(
        "/baike/yuchuli/step_event_graph",
        "post",
        this.$qs.stringify({
          // subjobid: ,
        ...
        res.data.table.map((item, key) => {
          item.num_id = this.pageNum * (this.table_current_1 - 1) + key + 1;
        });
        // 表数据
    ...
    NextSteps() {
      ...
      this.$router.push("GenerateTaskTwo");
    },
    //返回
    ReturnSuperior() {

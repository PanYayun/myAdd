      ...
      <div class="content_l">
        ...
        <div style="padding:0 16px;box-sizeing:border-box;">
          <div class="content_l_title_search">
            <Select v-if="refresh" v-model="treeSearchVal" style="width:150px;" @on-change="tree_change"
              default-label="treeSearchVal" placeholder="已存在知识体系">
              <Option v-for="item in treeSearchList" :value="item.value" :key="item.value">{{ item.label }}
                <Icon class="clear_cir" style="" type="md-close" @click="delete_tree" />
          ...
          <div class="filterInput" style="margin-top: 10px;width: 80%;">
            <Input prefix="ios-search" v-model="searchName" clearable placeholder="请输入名称" @on-enter="searchTree" />
          </div>
          <div style="">
  ...
  export default {
    ...
    methods: {
      ...
      searchTree() {
        ...
        let exist = false;
        let treeDataRaw = this.firstPageInitData;
        // 重置选中
        ...
        for (var i = 0; i < treeDataRaw.length; i++) {
          if (treeDataRaw[i].className.indexOf(this.searchName) != -1) {
            ...
            this.treeData = this.filterTreeData(handTreeData);
            break;
      ...
      // 删除体系树
      deleteSysName(txname) {
        // 获取已存在知识体系
        this.$axios("/baike/standServer/delete", "post", {
      ...
      nodeSave() {
        ...
        handleData.forEach((item1) => {
          ...
          if (!item1.hasOwnProperty('originId')) {
            item1['originId'] = item1.id;
          }
        });
      ...
      setTreeData(msg) {
        // 纳入之后更新
        if (msg.hasOwnProperty('nodeSaveName')) {
          this.existSysName(msg.nodeSaveName);
          return;
        }
        ...
        let beforeEdit = h(
          ...
          [
            h("span", [
              ...
              h("span", (parId == -1 || parId == 0 || parId == null) ? data.classNameNew : data.classNameNew + '(' +
                data.count + ')')
            ]),
          ]);
        return beforeEdit
            ...
            h("span", [
              ...
              h('span', {
                ...
                h(`${ data.isEdit ? '' : 'span'}`, {
                  ...
                  }, (parId == -1 || parId == 0 || parId == null) ? data.classNameNew : data.classNameNew + '(' +
                  data.count + ')'),
      ...
      toUp(root, node, data) {
        ...
        const upSysCode = parent.children[index - 1].hasOwnProperty('sysCode') ? (parent.children[index - 1]
          .classNoNew || parent.children[index - 1].sysCode) : (parent.children[index - 1].classNoNew || parent
          .children[index - 1].classNo);
        const currentSysCode = parent.children[index].hasOwnProperty('sysCode') ? (parent.children[index].classNoNew ||
          parent.children[index].sysCode) : (parent.children[index].classNoNew || parent.children[index].classNo);
      ...
      toDown(root, node, data) {
        ...
        const downSysCode = parent.children[index + 1].hasOwnProperty('sysCode') ? (parent.children[index + 1]
          .classNoNew || parent.children[index + 1].sysCode) : (parent.children[index + 1].classNoNew || parent
          .children[index + 1].classNo);
        const currentSysCode = parent.children[index].hasOwnProperty('sysCode') ? (parent.children[index].classNoNew ||
          parent.children[index].sysCode) : (parent.children[index].classNoNew || parent.children[index].classNo);
      ...
      handleDrop(root, node, data) {
        ...
        // 保存下发数据处理
        this.firstPageInitData.forEach((item) => {
          if (item.id == this.dragstartNode.node.id) {
            let parIdKey = item.hasOwnProperty('pId') ? 'pId' : 'parentId';
            item[parIdKey] = node.node.id;
          }
        });
      },
      // 右侧条件数据筛选
      ...
      delete_tree(event) {
        event.stopPropagation();
        this.deleteSysName(event.target.parentNode.innerText);

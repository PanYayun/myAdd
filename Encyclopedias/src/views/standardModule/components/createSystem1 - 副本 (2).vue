        ...
        methods: {
            ...
            showModal(data) {
                ...
                if (data && data != "新建") {
                    ...
                    //回显体系名称 应该禁用
                    this.fileName = this.newSaveTreeData[0].nameOld;
                    // 父页面操作节点后所有树节点的数组
            ...
            examName() {
                if (!this.fileName) {
                    ...
                    let oldTreeList = [];
                    let newTreeList = [];
                    // let appendData=this.selectData['append']?this.selectData['append']:[];
                    ...
                    newTreeList.forEach((item) => {
                        ...
                        interDataArr.forEach((item1) => {
                            if (item1.pId == 0 || item1.pId == -1) {
                                item1['pId'] = rootName;
                            }
                            item1.children = [];
                            if (item.id == item1.id) {
                                newSelInterData.push(item1);
                            }
                        });
                    });
                    ...
                    if(PpselectData&&PpselectData.length!=0){
                        PpselectData.forEach((item, index) => {
                        let {
                            children,
                            ...params
                        } = item

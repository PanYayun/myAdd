<template>
  <div>
    ...
    <!-- 纳入页面 -->
    <create-system1 ref="createSystem1" ></create-system1>
...
import createSystem1 from "./components/createSystem1"; //创建新体系
...
export default {
  ...
  methods: {
      ...
      showSys(){
        // 已选哪些节点
        let nodeArr = this.selectData;
        if(nodeArr.length==0){
          this.$Message.error("未选择标准，不可使用纳入功能;");
          return;
        }
        nodeArr.forEach((item) => {
          item.title = item.sysName ? item.sysName : '(分类名称缺失)';
          item.sysCode = item.sysCode;
        ...
        this.$refs.createSystem1.showModal('纳入', naruParam);
